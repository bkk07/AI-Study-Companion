...
export function QuizSetup({
  ...
  const start = () => {
    ...
    onStart({ scope, topicId: topicId ?? undefined, subtopicId: subId ?? undefined, conceptId: conceptId ?? undefined, numQuestions: qCount })
  }
            ...
            {recommended.map((c) => (
              <div key={c.concept_id} className="flex items-center gap-3 rounded-lg border border-indigo-100 bg-white px-3 py-2.5">
                ...
                <button
                  ...
                  disabled={busy}
                  onClick={() =>
                    onStart({ scope: "concept", conceptId: c.concept_id, numQuestions: qCount })
                  }
