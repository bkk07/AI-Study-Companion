...
## Hybrid PDF Page Routing & OCR (user-approved, in progress)

**Recorded:** 2026-09-16 before impl | Source: user "Implement Hybrid PDF Page Routing & OCR" (full spec in chat)
**Decisions:** LLM Inception (untouched), embeddings local (untouched), store Postgres+pgvector (untouched), parser PyMuPDF (kept), OCR Tesseract, Celery/background preserved, no new architecture.
**Scope:** per-page router in `document_extraction_service.py` (TEXT / OCR / EMPTY + `is_meaningful_text` floor 20 chars & 3 words, documented) → new `ocr_service.py` (`ocr_fitz_page`, lazy imports, OcrError) → `process_pdf` extracts once and reuses pages downstream (no double OCR in chunk path; result gains `ocr_pages`) → `chunk_pages` carries `methods` → `persist_chunks` stores `extraction_method` (new nullable column, NULL = legacy TEXT-era) → config `OCR_ENABLED/LANGUAGE/DPI` (defaults true/eng/300) → Dockerfile `tesseract-ocr` + `tesseract-ocr-eng`, requirements/pyproject `pytesseract` + `Pillow`, compose worker env + `.env.example`. Fail-fast replaced: only corrupt/zero-page/all-empty docs fail (OCR hint appended when every page OCR-failed); single-page/OCR failures degrade to EMPTY, never fail the doc. Statuses QUEUED/PROCESSING/READY/FAILED unchanged; existing idempotency/retry untouched.
**Files:** `services/ocr_service.py` (new), `services/document_extraction_service.py` (router), `worker/tasks/extraction.py`, `services/chunking_service.py`, `models/chunk.py`, migration `7e1b9c4a2d05`, `core/config.py`, `Dockerfile`, `requirements.txt`, `pyproject.toml`, `docker-compose.yml`, `.env.example`, `tests/test_hybrid_pdf_routing.py` (new, 12 tests incl. mocked-OCR scanned/mixed/diagram/empty/failure/disabled + worker e2e with RAG retrieval), `docs/*`.
**Verify:** 12 new tests + extraction/chunking/embedding-client/structure suites + robustness/pipeline-chain + embeddings/retrieval-isolation/knowledge-map/upload suites + `alembic check` + `docker build` (tesseract 5.5.0 live) + migration applied to dev DB; single commit.

### Hybrid-OCR Post-implementation (compact)

**Status:** ✅ Complete 2026-09-16
**Files:** as listed above (router + ocr_service + worker reuse + chunk method column + config/docker + 12 tests + docs)
**Verify:** 12/12 new green (incl. 158s worker→READY→retrieval e2e); reg1 31 green (extraction/chunking/embedding-client/structure); reg2 31 green (robustness/pipeline-chain); reg3 (embeddings/retrieval-isolation/knowledge-map/upload) green; `alembic check` clean; image `hy-ocr-check` runs `tesseract 5.5.0`; blank/corrupt PDFs still fail exactly as before.
**Known:** (1) `build_structure` re-reads the PDF via `extract_pages`, so scanned pages are OCR'd a second time there — accepted for smallest-diff; future fix is persisting normalized pages. (2) No vision pipeline: text+diagram pages stay TEXT with `has_images=True` recorded for the future vision branch. (3) Tesseract binary exists only in the Docker image; host test runs mock OCR (single seam `ocr_service.ocr_fitz_page`).
